...
function dynamic_icm_encoding_abs_nonormal_clean_fast_for_ils1!(
    X::AbstractMatrix{T},        
...
function optimized_least_squares_all!(
    a::AbstractMatrix{T},          
    X::AbstractMatrix{T},          
    C_contiguous::Array{T,3}, 
    B::AbstractMatrix{BType},              
    A_pool::Channel{Matrix{T}},  
    n::Int, m::Int, d::Int, h::Int 
)where {T <: AbstractFloat, BType <: Integer}
            ...
            try
                for i in start:stop
                    ...
                    @inbounds for j in 1:m
                        code_j = B_i[j]
                        col_j_ptr = pointer(C_contiguous, (code_j-1)*d + (j-1)*d*h + 1)

                        
                        a_i[j] = BLAS.dot(d, col_j_ptr, 1, pointer(X_i), 1)

                        
                        @inbounds for k in 1:j
                            code_k = B_i[k]
                            col_k_ptr = pointer(C_contiguous, (code_k-1)*d + (k-1)*d*h + 1)

                            dot_val = BLAS.dot(d, col_j_ptr, 1, col_k_ptr, 1)
                            A[j, k] = dot_val
                            if k != j
                                A[k, j] = dot_val  
                            end
...
function optimized_least_squares_single!(
    a_i::AbstractVector{T},          
    X_i::AbstractVector{T},          
    C_contiguous::Array{T,3}, 
    B_i::AbstractVector{BType},              
    A::Matrix{T},   
    m::Int, d::Int, h::Int 
)where {T <: AbstractFloat, BType <: Integer}
...
function _costs!(out::Vector{T},
                X::AbstractMatrix{T},
                Bm::AbstractMatrix{BType},
                am::AbstractMatrix{T},
                Cc::Array{T,3}) where {T<:AbstractFloat, BType<:Integer}
    d, n = size(X); m = size(Bm,1)
    @threads for j in 1:n
        s = zero(T)
...
function dynamic_icm_with_ils_abs_nonormal!(
    X::AbstractMatrix{T},                
    B::AbstractMatrix{BType},            
    a::AbstractMatrix{T},                
    rC::Matrix{T},               
    ...
    dynamic_icm_encoding_abs_nonormal_clean_fast_for_ils1!(X, B, a,rC,xC, C_contiguous, icm_iters; precomp=precomp,verbose=verbose)
    ...
    for outer in 1:ils_iters
        ...
        dynamic_icm_encoding_abs_nonormal_clean_fast_for_ils1!(X, B_cand, a_cand,rC,xC, C_contiguous, icm_iters; precomp=precomp,verbose=verbose)
