    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int t = 1; t < m; ++t) {
            ...
            for (int flat = 0; flat < H; ++flat) {
                ...
                if (!available[static_cast<std::size_t>(layer)]) continue;
                const float adj =
                    rC[static_cast<std::size_t>(flat)] * pre.invnorm_flat[static_cast<std::size_t>(flat)];
...
int main() {
    ...
    for (int i = 0; i < n; ++i) forced[static_cast<std::size_t>(i)] = uni_root(rng);

    // CPU xC baseline.
    stlq::ColMajorMatrix<float> xC_cpu(pre.H, n);
    stlq::Gemm(true, false, 1.0f, pre.C_all, X, 0.0f, &xC_cpu);

    // CPU reference: CPU greedy + CPU ICM/ILS using CPU xC.
    stlq::ColMajorMatrix<stlq::FullCode> B_init;
    stlq::ColMajorMatrix<float> a_init;
    GreedyInitAbsForcedRootCpu(pre, xC_cpu, forced, &B_init, &a_init);

    stlq::ColMajorMatrix<stlq::FullCode> B_cpu = B_init;
    stlq::ColMajorMatrix<float> a_cpu = a_init;
    std::vector<float> Xn2_cpu;
    std::vector<float> cost_cpu;
    stlq::DynamicIcmWithIlsAbsNoNormal(X, pre, xC_cpu, icm_iters, ils_iters, perturb_k, seed,
    ...
    if (cudaMalloc(&d_xC, xC_bytes) != cudaSuccess ||
    ...
    if (cudaMemcpyAsync(d_xC, xC_cpu.data.data(), xC_bytes, cudaMemcpyHostToDevice, ctx->stream) != cudaSuccess ||
        ...
        cudaMemcpyAsync(d_B, B_init.data.data(), B_bytes, cudaMemcpyHostToDevice, ctx->stream) != cudaSuccess ||
        cudaStreamSynchronize(ctx->stream) != cudaSuccess) {
        std::cerr << "H2D copy failed\n";
        cleanup();
        return 6;
    }
    ...
    if (!stlq::LinkageEncodeBatchCudaForcedRootDeviceXcFromInitB(*ctx, pre, d_xC, d_norm2, d_forced,
                                                                 n, icm_iters, ils_iters, perturb_k, seed, 0,
                                                                 d_B, d_a, d_cost, ws, &err)) {
        std::cerr << "LinkageEncodeBatchCudaForcedRootDeviceXcFromInitB failed: " << err << "\n";
    ...
    int mism_B = 0;
    for (int i = 0; i < n; ++i) {
        for (int l = 0; l < m; ++l) mism_B += (B_cpu(l, i) != B_gpu(l, i)) ? 1 : 0;
    }
    double mean_cpu = 0.0;
    double mean_gpu = 0.0;
    for (int i = 0; i < n; ++i) {
    ...
    std::cout << "test_linkage_encode_forced_root_device_xc_from_init:\n";
    ...
    if (diff > 1e-3 || mism_B != 0) {
        std::cerr << "FAIL: forced-root device-xC-from-init CPU/GPU mismatch\n";
        return 1;
    }
    return 0;
#endif
